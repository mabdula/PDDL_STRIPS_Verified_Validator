;; This is a modified version of one of the original Genome Edit Distance domains by Patrik Haslum that can be found at: http://users.cecs.anu.edu.au/~patrik/pddl/ged.tar.bz2
...
(define (domain genome-edit-distance)
  ...
  ;; :name x-inverted
  ;; :vars (?x)
  ;; :set-constraint (or (and (normal ?x) (not (inverted ?x))) (and (not (normal ?x)) (inverted ?x)))
  ;; )

  ;; (:invariant
  ;; :name x-s-first
  ;; :vars (?x ?y)
  ;; :set-constraint (or (not (s-first ?x)) (not (s-first ?y)))
  ;; )

  ;; (:invariant
  ;; :name x-s-first-snd
  ;; :vars (?x ?y)
  ;; :set-constraint (or (or (not (s-first ?x)) (not (s-first ?y))) (or (not (s-first ?x)) (not (idle))) (or (not (s-first ?x)) (not (finished))) (or (not (idle)) (not (finished)))))


  ;; (:invariant
  ;; :name x-s-last
  ;; :vars (?x ?y)
  ;; :set-constraint (or (not (s-last ?x)) (not (s-last ?y)))
  ;; )
  
  ;; (:invariant
  ;; :name x-last-cut
  ;; :vars (?x ?y)
  ;; :set-constraint (or (not (last-cut-point ?x)) (not (last-cut-point ?y)))
  ;; )
  
  ;; (:invariant
  ;; :name x-cut-point-1
  ;; :vars (?x ?y)
  ;; :set-constraint (or (not (cut-point-1 ?x)) (not (cut-point-1 ?y)))
  ;; )
  
  ;; (:invariant
  ;; :name x-splice-point-1
  ;; :vars (?x ?y)
  ;; :set-constraint (or (not (splice-point-1 ?x)) (not (splice-point-1 ?y)))
  ;; )
  
  ;; (:invariant
  ;; :name x-splice-point-2
  ;; :vars (?x ?y)
  ;; :set-constraint (or (not (splice-point-2 ?x)) (not (splice-point-2 ?y)))
  ;; )
